...
export default function Portfolio({ properties }) {
  // DİKKAT: Burada artık isMenuOpen state'i yok, çünkü Layout hallediyor.
  ...
  return (
    <>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .prop-card { background: #0d223f; border: 1px solid rgba(212,175,55,0.1); overflow: hidden; transition: 0.3s; text-decoration: none; display: flex; flexDirection: column; }
        .prop-card:hover { border-color: #d4af37; transform: translateY(-5px); }
        @media (max-width: 768px) { .prop-grid { grid-template-columns: 1fr; } }
      `}} />

      {/* BURADA NAV YOK - LAYOUT OTOMATİK BASIYOR */}
      ...
      {/* BURADA FOOTER YOK - LAYOUT OTOMATİK BASIYOR */}
    </>
  );
}
