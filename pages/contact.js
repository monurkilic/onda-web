...
// Sosyal Medya İkon Bileşeni (Footer İçin)
...
export default function Contact() {
  ...
  return (
    <div style={{backgroundColor: '#0a192f', color: '#fff', minHeight: '100vh', fontFamily: 'serif', display: 'flex', flexDirection: 'column'}}>
      ...
      <main style={{flex: 1, padding: '60px 20px', maxWidth: '1000px', margin: '0 auto'}}>
        <h2 style={{color: '#d4af37', textAlign: 'center', marginBottom: '40px', letterSpacing: '3px'}}>İLETİŞİM</h2>
        <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '40px'}}>
           <div>
             <h4 style={{color: '#d4af37'}}>OFİSİMİZ</h4>
             <p style={{color: '#ccc', lineHeight: '1.8'}}>İzmir - Ankara<br/>m.onur.kilic@gmail.com<br/>+90 532 646 69 09</p>
           </div>
           <form action="https://formspree.io/f/xvzvooyy" method="POST" style={{display: 'flex', flexDirection: 'column', gap: '15px'}}>
             <input type="text" name="name" placeholder="Adınız" required style={{padding: '12px', background: '#0d223f', border: '1px solid #d4af37', color: '#fff'}} />
             <input type="email" name="email" placeholder="E-posta" required style={{padding: '12px', background: '#0d223f', border: '1px solid #d4af37', color: '#fff'}} />
             <textarea name="message" placeholder="Mesajınız" rows="5" required style={{padding: '12px', background: '#0d223f', border: '1px solid #d4af37', color: '#fff'}}></textarea>
