        ...
        <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: '40px'}}>
          {properties && properties.map((item) => (
            <div key={item._id} style={{background: '#0d223f', border: '1px solid #d4af3722', overflow: 'hidden', borderRadius: '2px'}}>
              {item.mainImage ? (
                <img src={urlFor(item.mainImage).width(800).url()} alt={item.title} style={{width: '100%', height: '250px', objectFit: 'cover'}} />
              ) : (
              ...
              <div style={{padding: '30px'}}>
                ...
                <div style={{padding: '20px', background: 'rgba(212, 175, 55, 0.03)', borderLeft: '2px solid #d4af37', marginTop: '20px'}}>
                  <p style={{fontStyle: 'italic', fontSize: '0.9rem', color: '#ccc', lineHeight: '1.6', margin: 0}}>"{item.analysis}"</p>
                </div>
              </div>
            </div>
          ))}
