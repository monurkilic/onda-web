  ...
  return (
    <div style={{backgroundColor: '#0a192f', color: '#fff', minHeight: '100vh', fontFamily: 'serif', display: 'flex', flexDirection: 'column'}}>
      ...
      <div className={`mobile-overlay ${isMenuOpen ? 'active' : ''}`}>
        ...
        <a href="/portfolio" style={{color: '#d4af37', fontSize: '1.8rem', textDecoration: 'none'}} onClick={() => setIsMenuOpen(false)}>PORTFÖY</a>
      ...
      {/* LIST CONTENT */}
      <main style={{flex: 1}}>
        <h1 style={{textAlign: 'center', color: '#d4af37', margin: '60px 0 20px 0', fontSize: '2rem', letterSpacing: '4px', fontWeight: '300'}}>SEÇKİN PORTFÖYÜMÜZ</h1>
        <div className="grid-layout">
          {properties && properties.map((item) => (
            <div key={item._id} style={{background: '#0d223f', border: '1px solid rgba(212, 175, 55, 0.1)', overflow: 'hidden'}}>
              <a href={`/portfolio/${item.slug?.current}`} style={{textDecoration: 'none', color: 'inherit'}}>
                ...
                <div style={{padding: '25px'}}>
                  <h3 style={{fontSize: '1.2rem', marginBottom: '10px', color: '#fff', fontWeight: '400'}}>{item.title}</h3>
                  <p style={{color: '#d4af37', fontSize: '1.5rem', fontWeight: 'bold'}}>{item.price} {item.currency}</p>
                  <div style={{fontSize: '0.8rem', color: '#8e8e8e', marginTop: '10px', letterSpacing: '1px'}}>{item.location}</div>
                </div>
              </a>
            </div>
          ))}
