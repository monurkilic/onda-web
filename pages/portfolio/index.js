  ...
  return (
    <div style={{backgroundColor: '#0a192f', color: '#fff', minHeight: '100vh', fontFamily: 'serif', display: 'flex', flexDirection: 'column'}}>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        .grid-l { display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 30px; padding: 40px 20px; max-width: 1200px; margin: 0 auto; }
        ...
        @media (max-width: 768px) { .desktop-nav { display: none !important; } .hamburger { display: flex !important; } .grid-l { grid-template-columns: 1fr !important; } }
      `}} />
      ...
      <main style={{flex: 1}}>
        <h1 style={{textAlign: 'center', color: '#d4af37', margin: '40px 0', fontWeight: '300'}}>SEÇKİN PORTFÖYÜMÜZ</h1>
          ...
          {properties && properties.map((item) => (
            <div key={item._id} style={{background: '#0d223f', border: '1px solid rgba(212, 175, 55, 0.1)'}}>
              <a href={`/portfolio/${item.slug?.current}`} style={{textDecoration: 'none', color: 'inherit'}}>
                <img src={urlFor(item.mainImage).width(600).url()} style={{width: '100%', height: '240px', objectFit: 'cover'}} alt="i" />
                <div style={{padding: '20px'}}><h3 style={{fontSize: '1.1rem', color: '#fff'}}>{item.title}</h3><p style={{color: '#d4af37', fontSize: '1.3rem', fontWeight: 'bold'}}>{item.price} {item.currency}</p></div>
              </a>
            </div>
          ))}
        </div>
