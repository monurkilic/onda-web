  ...
  return (
    <div style={{backgroundColor: '#0a192f', color: '#fff', minHeight: '100vh', fontFamily: 'serif', overflowX: 'hidden'}}>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .mobile-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: #0a192f; z-index: 1000; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 30px; transform: translateX(100%); transition: 0.4s; }
        .mobile-overlay.active { transform: translateX(0); }
        @media (max-width: 768px) { .hero-t { font-size: 1.8rem !important; } .container { padding: 20px 15px; } .gal-grid { grid-template-columns: repeat(2, 1fr); } .hamburger { display: flex !important; } }
      `}} />
      ...
      {photoIndex !== null && (
        <div onClick={() => setPhotoIndex(null)} style={{position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.95)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
          <button onClick={prev} style={{position: 'absolute', left: '15px', background: 'none', color: '#d4af37', fontSize: '40px', border: 'none'}}>‹</button>
          <img src={urlFor(allPhotos[photoIndex]).width(1200).url()} style={{maxWidth: '90%', maxHeight: '80%'}} alt="p" />
          <button onClick={next} style={{position: 'absolute', right: '15px', background: 'none', color: '#d4af37', fontSize: '40px', border: 'none'}}>›</button>
        </div>
      )}
      ...
      <nav style={{padding: '15px 30px', borderBottom: '1px solid rgba(212,175,55,0.1)', background: '#0a192f', position: 'sticky', top: 0, zIndex: 100, display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
        <a href="/" style={{display: 'flex', alignItems: 'center', gap: '8px', textDecoration: 'none'}}>
          ...
          <span style={{color: '#d4af37', fontWeight: 'bold'}}>ONDA</span>
        </a>
      ...
      <main className="container">
        ...
        <div onClick={() => setPhotoIndex(0)} style={{cursor: 'zoom-in'}}><img src={urlFor(property.mainImage).width(1200).url()} style={{width: '100%'}} alt="m" /></div>
        <div className="gal-grid">
          {property.gallery && property.gallery.map((img, i) => (
            <div key={i} onClick={() => setPhotoIndex(i + 1)} style={{cursor: 'zoom-in', height: '140px', overflow: 'hidden'}}>
              <img src={urlFor(img).width(600).url()} style={{width: '100%', height: '100%', objectFit: 'cover'}} alt="g" />
            </div>
          ))}
        </div>
        <div style={{margin: '50px 0', textAlign: 'center'}}>
          <div style={{fontSize: '2.2rem', fontWeight: 'bold', marginBottom: '30px'}}>{property.price} {property.currency}</div>
          <div style={{padding: '30px', background: 'rgba(212, 175, 55, 0.03)', borderLeft: '4px solid #d4af37', textAlign: 'left'}}>
            <h4 style={{color: '#d4af37', marginTop: 0}}>ONDA ANALİZİ</h4>
            <p style={{fontStyle: 'italic', color: '#ccc'}}>"{property.analysis}"</p>
          </div>
        </div>
        <div style={{display: 'flex', justifyContent: 'center', marginBottom: '60px'}}>
          <a href={waLink} target="_blank" rel="noreferrer" style={{padding: '18px 40px', background: '#25D366', color: '#fff', textDecoration: 'none', fontWeight: 'bold', borderRadius: '50px'}}>WHATSAPP İLE SOR</a>
        </div>
      </main>

      <footer style={{padding: '40px', borderTop: '1px solid rgba(212,175,55,0.1)', textAlign: 'center', opacity: 0.4}}>© 2026 ONDA YATIRIM</footer>
    </div>
  );
}
