...
export default function BlogIndex({ posts }) {
  ...
  return (
    <div style={{backgroundColor: '#0a192f', color: '#fff', minHeight: '100vh', fontFamily: 'serif', display: 'flex', flexDirection: 'column', overflowX: 'hidden'}}>
      <Head><title>Blog | Onda Yatırım Analizleri</title><meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=0" /></Head>
      <style dangerouslySetInnerHTML={{ __html: `
        * { box-sizing: border-box; }
        ...
        @media (max-width: 768px) { .desktop-nav { display: none !important; } .hamburger { display: flex !important; } .blog-grid { grid-template-columns: 1fr !important; } .b-title { font-size: 1.8rem !important; } }
      `}} />
      ...
      <nav style={{padding: '20px 30px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(212,175,55,0.1)', background: '#0a192f', position: 'sticky', top: 0, zIndex: 1000}}>
        <a href="/" style={{display: 'flex', alignItems: 'center', gap: '10px', textDecoration: 'none'}}><img src="/logo.png" style={{height: '35px'}} alt="L" /><span style={{color: '#d4af37', fontWeight: 'bold', letterSpacing: '2px'}}>ONDA</span></a>
        <div className="desktop-nav" style={{display: 'flex', gap: '20px', fontSize: '0.8rem'}}>
          ...
          <a href="/blog" style={{color: '#d4af37', textDecoration: 'none'}}>BLOG</a>
        ...
        <div className="hamburger" onClick={() => setIsMenuOpen(!isMenuOpen)}><div></div><div></div><div></div></div>
      </nav>
      ...
      <div className={`mobile-overlay ${isMenuOpen ? 'active' : ''}`}>
        ...
        <a href="/blog" style={{color: '#d4af37', fontSize: '1.8rem', textDecoration: 'none'}}>BLOG</a>
        <a href="/valuation" style={{color: '#fff', fontSize: '1.8rem', textDecoration: 'none'}}>MÜLK DEĞERLEME</a>
        <a href="/contact" style={{color: '#fff', fontSize: '1.8rem', textDecoration: 'none'}}>İLETİŞİM</a>
      </div>

      <main style={{flex: 1}}>
        <h1 className="b-title" style={{textAlign: 'center', color: '#d4af37', margin: '60px 0 20px 0', letterSpacing: '4px', fontWeight: '300'}}>ONDA ANALİZLERİ</h1>
        <p style={{textAlign: 'center', color: '#8e8e8e', marginBottom: '40px', padding: '0 20px'}}>Gayrimenkul dünyasına rasyonel ve psikolojik bir bakış.</p>
        
        <div className="blog-grid">
          {posts && posts.map((post) => (
            <a href={`/blog/${post.slug?.current}`} key={post._id} style={{textDecoration: 'none', background: '#0d223f', border: '1px solid rgba(212,175,55,0.1)', overflow: 'hidden', display: 'flex', flexDirection: 'column'}}>
              ...
              <div style={{padding: '25px', flex: 1}}>
                ...
                <span style={{color: '#d4af37', fontSize: '0.8rem', fontWeight: 'bold', letterSpacing: '1px'}}>DEVAMINI OKU →</span>
              </div>
            </a>
          ))}
        </div>
